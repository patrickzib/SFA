...
public abstract class Classifier {
    ...
    public static double formatError(int correct, int testSize) {
        double error = Math.round(1000 * (testSize - correct ) / (double)(testSize))
                       / 1000.0;
        return error;
    }

    @SuppressWarnings("unchecked")
    public static TimeSeries[][] getStratifiedSplits(TimeSeries[] samples, int splits) {
        ...
        for (int s = 0; s < splits; s++) {
            ...
            for (Entry<String, LinkedList<Integer>> data : elements.entrySet()) {
                ...
                while (!data.getValue().isEmpty() && i <= count) {
